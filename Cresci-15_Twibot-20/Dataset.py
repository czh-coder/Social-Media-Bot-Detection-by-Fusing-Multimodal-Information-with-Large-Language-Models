...
class Twibot20(Dataset):
    ...
    def get_data(self):
        labels=torch.load(self.root+"label_merge.pt").to(self.device)
        des_tensor=torch.load(self.root+"des_tensor_merge.pt").to(self.device)
        tweets_tensor=torch.load(self.root+"tweets_tensor_merge.pt").to(self.device)
        num_prop=torch.load(self.root+"num_properties_tensor_merge.pt").to(self.device)
        category_prop=torch.load(self.root+"cat_properties_tensor_merge.pt").to(self.device)
        edge_index=torch.load(self.root+"edge_index_merge.pt").to(self.device)
        edge_type=torch.load(self.root+"edge_type_merge.pt").to(self.device)
        train_idx=torch.load(self.root+"train_idx_merge.pt").to(self.device)
        val_idx=torch.load(self.root+"val_idx_merge.pt").to(self.device)
        test_idx=torch.load(self.root+"test_idx_merge.pt").to(self.device)
